...
const Register = ({ email, setEmail, password, setPassword, city, setCity, zip, setZip, state, setState, streetAddress, setStreetAddress }) => {
  async function handleSubmit(event) {
    ...
    const result = await register(email, password, streetAddress, city, state, zip);
    if(result) {
  ...
  return (
    <div className="form-container">
      <form className="login-form" onSubmit={handleSubmit}>
        ...
        <label className="label">
          Street Address:
          <input type="text" placeholder="street address" value={streetAddress} onChange={(event) => {setStreetAddress(event.target.value)}}></input>
        </label>
        <label className="label">
          City:
          <input type="text" placeholder="city" value={city} onChange={(event) => {setCity(event.target.value)}}></input>
        </label>
        <label className="label">
          State:
          <input type="text" placeholder="state" value={state} onChange={(event) => {setState(event.target.value)}}></input>
        </label>
        <label className="label">
          Zip:
          <input type="text" placeholder="zip" value={zip} onChange={(event) => {setZip(event.target.value)}}></input>
        </label>
